...
fn main() {
    ...
    let omega0 = Frequency::new(f64::from(TAU));
    let q_factor: f64 = 5.0;

    // k = m·ω₀²  (N/m)
    let spring_k = mass * pow!(omega0, 2);
    // c = m·ω₀/Q  (N·s/m)
    let damping_c = mass * omega0.map(|w| w / q_factor);

    let x0 = Meters::new(0.10);
    let v0 = Velocity::new(0.0);

    // a = -(k·x + c·v)/m
    let a0 = (-spring_k * x0 - damping_c * v0) / mass;

    let zeta: f64 = 1.0 / (2.0 * q_factor);
    let omega_d = omega0.map(|w| w * (1.0 - zeta * zeta).sqrt());

    // E = ½kx² + ½mv²
    let e0 = (spring_k * pow!(x0, 2)).map(|e| e * 0.5) + (mass * pow!(v0, 2)).map(|e| e * 0.5);
    ...
    let dt = Seconds::new(0.001);
    let t_total = Seconds::new(5.0);
    let n_steps = (t_total / dt).value as usize;
    ...
    for step in 0..=n_steps {
        if step % print_every == 0 {
            ...
            let e = (spring_k * pow!(x, 2)).map(|val| val * 0.5)
        ...
        if step < n_steps {
            ...
            let x_new = x + v * dt + (a * dt * dt).map(|val| val * 0.5);
            let v_half = v + (a * dt).map(|val| val * 0.5);
            let a_new = (-spring_k * x_new - damping_c * v_half) / mass;
            let v_new = v + ((a + a_new) * dt).map(|val| val * 0.5);
    ...
    let x_analytic = x0 * exp_arg * cos_arg.cos();
    let x_error = (x - x_analytic).map(|diff| diff.abs());
